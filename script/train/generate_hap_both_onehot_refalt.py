...
def msa(seq_list, ref, mincov, maxcov):
    ...
    if len(sample) < mincov:
        return np.zeros((n, 5)), np.zeros((n, 5))
    ...
    return ref_real_0_mat[:n, :], alt_mat[:n, :]
...
def run(cp0, fastafile, samfile, pileup_file,miss_output, hap_reads_0, hap_reads_1, n):
    mincov = 1
    maxcov = 100
    ...
    for pcol in samfile.pileup(chrom, v_pos - 1, v_pos, min_base_quality=12, min_mapping_quality=28, flag_filter=flag,
        ...
        if len(d[0]) == 0:
            ref_data_0, data_0 = np.zeros((ln, 5)), np.zeros((ln, 5))
        else:
            ref_data_0, data_0 = msa(d[0], ref, 2, 100)
        if len(d[1]) == 0:
            ref_data_1, data_1 = np.zeros((ln, 5)), np.zeros((ln, 5))
        else:
        ...
        #     continue
        in_run = 1
        s = chrom + ' ' + str(v_pos) + ' ' + ' '.join(str(x) for x in data.reshape(-1).astype(np.int16)) + ' ' + str(
            n) + '\n'
        pileup_file.write(s)
    if in_run == 0:
        s = chrom + ' ' + str(v_pos) + '\n'
...
def gen(args):
    ...
    miss_output = open(args.out_path + '/' + 'miss' + mode + '.txt', "a")
    samfile = pysam.Samfile(sam_path, "rb")
    fastafile = pysam.FastaFile(ref_path)
    kf = open(fb, 'r')
    if fb.split('/')[-1].split('_')[0] == 'f' or fb.split('/')[-1].split('_')[0] == 'fs' or \
            fb.split('/')[-1].split('_')[0] == 'c' or fb.split('/')[-1].split('_')[0] == 'e':
        n = 0
    if fb.split('/')[-1].split('_')[0] == 'p' or fb.split('/')[-1].split('_')[0] == 'm':
        n = 1
    ...
    for chr_pos in kf:
        run(chr_pos, fastafile, samfile, pileup_file,miss_output, hap_reads_0, hap_reads_1, n)
...
def main():
    ...
    parser.add_argument('--ref_path', '-r', type=str,
                        default='/home/gyc/ont/seq/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna',
    ...
    parser.add_argument('--mode', '-m', type=str, default=None,
                        help="snp或者indel. (default: %(default)s)")
    parser.add_argument('--out_path', '-o', type=str, default=None,
                        help="输出文件的路径. (default: %(default)s)")
    parser.add_argument('--pf_class', '-l', type=str, default=None,
                        help="输入位点文件的类型. (default: %(default)s)")
    ...
    if args.chromosome != 'chrX' and args.chromosome != 'chrY':
        gen(args)
